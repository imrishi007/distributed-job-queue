...
- Runs a worker registry: each `djq-worker` gets a stable id and heartbeats (`voluntary`/`busy`) into PostgreSQL, and `GET /workers` returns the live fleet — an absent heartbeat (`last_seen` going stale) is how a dead worker is distinguished from a busy one. A backend housekeeping thread deletes registrations whose heartbeat has been silent for the configured TTL (default 120s, `DJQ_WORKER_TTL_SECONDS`), so vanished workers are eventually reaped from the fleet.
...
- Reads connection settings from the environment (`DJQ_PG_DSN`, `DJQ_REDIS_HOST`/`DJQ_REDIS_PORT`, `DJQ_BIND`, and `DJQ_WORKER_TTL_SECONDS` for how long a silent worker stays in the registry) with bare-metal defaults, so the very same binaries run on a host or inside a container.
